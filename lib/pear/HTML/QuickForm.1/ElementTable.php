 ...
 *
 * @category HTML
 * @package  HTML_QuickForm_ElementTable
 * @license  http://www.gnu.org/copyleft/lesser.html  LGPL
 * @author   Justin Patrin <papercrane@reversefold.com>
 * @version  $Id: ElementTable.php,v 1.1.1.1 2006/04/17 03:36:24 papercrane Exp $
 */
 ...
 */
class HTML_QuickForm_ElementTable extends HTML_QuickForm_element {
     ...
     */
    function HTML_QuickForm_ElementTable($name = null, $label = null/*, $columnNames = null,
                                         $rowNames = null, $rows = null, $attributes = null*/,
                                         $options = null)
    {
        parent::HTML_QuickForm_element($name, $label);
        $this->updateAttributes(array('class' => 'elementTable'));
        //$this->setRows($rows);
    ...
    /**
     * Freezes all elements in the table
     */
    function freeze()
    {
        parent::freeze();
    ...
    function toHtml()
    {
        ...
        if ($this->_columnNames) {
            ...
            $thead->updateRowAttributes(0, array('class' => 'elementTableColumnLabel'), true);
        }

        $row = 0;
        foreach (array_keys($this->_rows) as $key) {
            $col = 0;
            $tbody->setHeaderContents($row, $col, $this->_rowNames[$key]);
            foreach (array_keys($this->_rows[$key]) as $key2) {
                ++$col;
        ...
        $tbody->updateColAttributes(0, array('class' => 'elementTableRowLabel'));
        return $table->toHTML();
...
HTML_QuickForm::registerElementType('elementTable', __FILE__, 'HTML_QuickForm_ElementTable');
