...
class HTML_QuickForm_CAPTCHA extends HTML_QuickForm_input
{
    ...
    function toHtml()
    {
        ...
        $html = $tabs . '<a href="' . $callback . '" target="_blank" ' . $this->_getAttrString($this->_attributes) . ' onclick="var cancelClick = false; ' . $this->getOnclickJs($imgName) . ' return !cancelClick;"><img src="' . $callback . '" name="' . $imgName . '" id="' . $imgName . '" width="' . $width . '" height="' . $height . '" title="' . htmlspecialchars($alt) . '" /></a>';
