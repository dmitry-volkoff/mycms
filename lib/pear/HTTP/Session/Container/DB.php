 ...
 * @version    CVS: $Id: DB.php,v 1.6 2006/09/17 08:41:02 troehr Exp $
...
class HTTP_Session_Container_DB extends HTTP_Session_Container
{
    ...
    function read($id)
    {
        $query = sprintf("SELECT sdata FROM %s WHERE id = %s AND expiry >= %d",
                         $this->options['table'],
    ...
    function write($id, $data)
    {
        if ((false !== $this->crc) && ($this->crc === strlen($data) . crc32($data))) {
            ...
            if (0 == intval($result)) {
                ...
                $query = sprintf("INSERT INTO %s (id, expiry, sdata) VALUES (%s, %d, %s)",
                                 $this->options['table'],
                                 $this->db->quoteSmart(md5($id)),
                                 time() + ini_get('session.gc_maxlifetime'),
                ...
                // Update existing row
                $query = sprintf("UPDATE %s SET expiry = %d, sdata = %s WHERE id = %s",
                                 $this->options['table'],
                                 time() + ini_get('session.gc_maxlifetime'),
                                 $this->db->quoteSmart($data),
                                 $this->db->quoteSmart(md5($id))
    ...
    function replicate($targetTable, $id = null)
    {
        ...
        if (0 == intval($result)) {
          ...
          // Update existing row
            ...
            $query = sprintf("UPDATE %s dst, %s src SET dst.expiry = src.expiry, dst.sdata = src.sdata WHERE dst.id = src.id AND src.id = %s",
                             $targetTable,
                             $this->options['table'],
                             $this->db->quoteSmart(md5($id))
                            );
